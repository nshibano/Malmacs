...
type type_info = 
    { ti_name : string
      ...
      ti_res : type_expr // always Tconstr
      ti_kind : type_kind }
...
type label_info = 
    { li_id : type_id // ID_FOOBAR
      ...
      li_arg : type_expr // Tconstr (ID_LIST, [Tvar 'a])
      li_res : type_expr // Tconstr (ID_FOOBAR, [Tvar 'a])
...
// Example in field comment is for the 'Foo' case when we define type 'foobar' as below.
// "type ('a, 'b) foobar = Foo of 'a | Bar of 'b"
type constr_info = 
    { ci_params : type_var list // ['a; 'b]
      ci_args : type_expr list // [Tvar 'a]
      ci_res : type_expr //  Tconstr (ID_FOOBAR, [Tvar 'a; Tvar 'b])
      ci_tag : int // 0
    }
...
let tv_a = { link = None; level = generic_level }
let tv_b = { link = None; level = generic_level }
let ty_a = Tvar tv_a
let ty_b = Tvar tv_b
...
let ty_bb = arrow ty_bool ty_bool
let ty_bbb = arrow2 ty_bool ty_bool ty_bool
let ty_vvi = arrow2 ty_a ty_a ty_int
let ty_vvb = arrow2 ty_a ty_a ty_bool
let ty_ss = arrow ty_string ty_string
...
let ti_list = make_ti type_id.LIST "list" [ tv_a ] (Kvariant [ "[]", 0, []; "::", 1, [ ty_a; Tconstr(type_id.LIST, [ ty_a ]) ] ])
...
let ti_lexbuf = make_ti type_id.LEXBUF "lexbuf" [] (Krecord [("lbSource", ty_string, Immutable)
                                                             ("lbStartPos", ty_int, Mutable)
                                                             ("lbEndPos", ty_int, Mutable)
                                                             ("lbScanStartPos", ty_int, Mutable)
                                                             ("lbEof", ty_bool, Mutable)])
...
let rec typeexpr_of_type (tyenv : tyenv) (bnds : Dictionary<Type, type_expr>) (arg_names : string list) (ty : Type) =
    if ty = typeof<unit> then
        ...
        | hd :: tl -> Tarrow (hd,(typeexpr_of_type tyenv bnds []) t1, (typeexpr_of_type tyenv bnds tl) t2)
...
let tyenv_basic, id_ref, id_option, tag_exn_Failure, tag_exn_Division_by_zero, tag_exn_Index_out_of_range, tag_exn_Invalid_argument, tag_exn_Match_failure =
    ...
    let tyenv, tag_exn_Failure = add_exn_constructor tyenv "Efailure" [ty_string]
    let tyenv, tag_exn_Division_by_zero = add_exn_constructor tyenv "EdivisionByZero" []
    let tyenv, tag_exn_Index_out_of_range = add_exn_constructor tyenv "EindexOutOfRange" []
    let tyenv, tag_exn_Invalid_argument = add_exn_constructor tyenv "EinvalidArgument" []
    let tyenv, tag_exn_Match_failure = add_exn_constructor tyenv "EmatchFailure" []

    tyenv, id_ref, id_option, tag_exn_Failure, tag_exn_Division_by_zero, tag_exn_Index_out_of_range, tag_exn_Invalid_argument, tag_exn_Match_failure