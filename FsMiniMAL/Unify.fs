...
/// Expand type abbreviation. Returned type is repr'ed.
let rec expanded (tyenv : tyenv) ty =
    let ty = repr ty
    match ty with
    | Tconstr(id, ty_args) ->
...
let rec same_type tyenv ty1 ty2 =
    match expanded tyenv ty1, expanded tyenv ty2 with
    ...
    | Tconstr (id1, l1), Tconstr (id2, l2) when id1 = id2 -> List.forall2 (same_type tyenv) l1 l2
    | _ -> false
