...
let rec pattern (alloc : alloc) (leftmost : bool) (pat : Syntax.pattern) : Value.pattern = 
    ...
    | SPlist (kind, l) -> 
        ...
        match kind with
        | LKlist ->
            let rec loop (l : pattern list) =
                match l with
                | [] -> UPint 0
                | hd :: tl -> UPblock (1, [| hd; loop tl |])
            loop ul
        | LKarray -> UParray(Array.ofList ul)
    | SPblock(tag, args) ->
...
and expression (alloc : alloc) (tailcall_info : (string * int) option) (se : Syntax.expression) : Value.code =
    ...
    let ue = 
        ...
        | SEconstr (tag, l) ->
            if l.Length = 0 then
                UEconst(of_int dummy_mm tag)
            else
                let uel = List.map (expression alloc None) l
                UEblock(tag, Array.ofSeq uel)
        | SEint s -> UEconst(Value.of_int dummy_mm (int s))
        | SEchar c -> UEconst(Value.of_char dummy_mm c)
        | SEfloat x -> UEconst (Value.of_float dummy_mm x)
